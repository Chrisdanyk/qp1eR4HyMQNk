...
public class UserService extends BaseService<UserEntity> implements Serializable {
    ...
    protected void handleDependenciesBeforeDelete(UserEntity user) {
        ...
        this.cutAllOfficiantProgrammesAssignments(user);
        
        this.cutAllPredicateurProgramme2sAssignments(user);
        
    }
    
    // Remove all assignments from all programme a user. Called before delete a user.
    @Transactional
    private void cutAllOfficiantProgrammesAssignments(UserEntity user) {
        entityManager
                .createQuery("UPDATE Programme c SET c.officiant = NULL WHERE c.officiant = :p")
                .setParameter("p", user).executeUpdate();
    }
    
    // Remove all assignments from all programme2 a user. Called before delete a user.
    @Transactional
    private void cutAllPredicateurProgramme2sAssignments(UserEntity user) {
        entityManager
                .createQuery("UPDATE Programme c SET c.predicateur = NULL WHERE c.predicateur = :p")
                .setParameter("p", user).executeUpdate();
    }
